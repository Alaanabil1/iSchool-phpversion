...
if (page.includes("register")) {
    if (page.includes("registerstep01")) {
    ...
    $(".scheduletableitem").click(function() {
        ...
        $("#b-button").prop("checked", false);
        $(this).addClass("active");
        $(".scheduletableitem.active #b-button").prop("checked", true);
        Num = $(this).index();
        $(".scheduletableitem:nth-child(" + (Num + 1) + ") .check").addClass(
            "active"
        );
    });
    ...
    function checkout() {
        ...
        location.href =
            return_page +
            "?orderStatus=PAID&statusCode=200&paymentMethod=PayUsingCC&merchantRefNumber=" +
            choosen_ref;
    }
    ...
    function detectDay() {
        if ($("#dayDate").val() < 1) {
            ...
            $("#dayDate").val(maxDays);
            $(".mb-3.date.days .days-slider li").removeClass("active");
            $(".mb-3.date.days .days-slider li:nth-child(" + maxDays + ")").addClass(
                "active"
            );
        } else {
            $(".mb-3.date.days .days-slider li").removeClass("active");
            $(
                ".mb-3.date.days .days-slider li:nth-child(" + $("#dayDate").val() + ")"
            ).addClass("active");
        }
    }
    ...
    function detectMonth() {
        if ($("#monthDate").val() < 1) {
            ...
            $(".mb-3.date.months .days-slider li").removeClass("active");
            $(
                ".mb-3.date.months .days-slider li:nth-child(" +
                $("#monthDate").val() +
                ")"
            ).addClass("active");
        }
    ...
    for (i = 1; i <= 13; i++) {
        Yearsarr.push(
            $(".mb-3.date.years .days-slider li:nth-child(" + i + ")").text()
        );
    }
    ...
    function genrateYears() {
        var date = new Date();
        for (
            i = Number(date.getFullYear() - 18); i <= Number(date.getFullYear() - 6); i++
        ) {
            $(".mb-3.date.years .days-slider").html(
                $(".mb-3.date.years .days-slider").html() + "<li>" + i + "</li>"
            );
        }
    }
    $(document).click(function() {
